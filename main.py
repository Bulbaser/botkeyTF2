...
apiLolz = LolzteamApi(lolzToken, idLolz)
...
async def start(message: types.Message):
    await message.answer(f'Добро пожаловать, {message.from_user.username}', reply_markup = keyboard.userButtonMenu)

#Команда /admin
@dp.message_handler(commands='admin')
async def admin(message: types.Message):
    if message.from_user.id == int('878562927') or message.from_user.id == int('1059195416'):
        await message.answer(f'Привет, {message.from_user.username}. Ты авторизовался в админке', reply_markup = keyboard.adminMenu)
    else:
        await message.answer('Не понял команды.')

#Работа с кнопками
@dp.message_handler(content_types = 'text')
async def mainCommands(message: types.Message):
    if message.text == 'Профиль':
        await message.answer(f'Имя : {message.from_user.username}\nБаланс : {see_user_money(message.from_user.id)}', reply_markup = keyboard.withdrawalOfFunds)
    elif message.text == 'Меню':
        await message.answer('Покупай/продавай ключи выгодно с нами!', reply_markup = keyboard.userMenu)

#Работа с инлайн-клавиатурой
@dp.callback_query_handler(lambda callback_query: True)
async def some_callback_handler(callback_query: types.CallbackQuery):
    if callback_query.data == 'withdraw':
        await bot.send_message(callback_query.from_user.id, text = 'Введи свой номер телефона QIWI и сумму денег на вывод(Пример : 79999999999 500)')
        ...
        statsSold, statsBuy = showStatsInBot()
        await bot.send_message(callback_query.from_user.id, text = f'''Количество проданных ключей : {statsSold}\nКоличество купленных ключей : {statsBuy}''')
    elif callback_query.data == 'numberOfKeys':
        try:
            await bot.send_message(callback_query.from_user.id, text =  f'На данный момент ключей на продаже: {inv_steam()}')
        except AttributeError:
            await bot.edit_message_text(callback_query.from_user.id, text = 'Ключей нет')
            logging.error(f'{callback_query.from_user.id} | {callback_query.text}')
    elif callback_query.data == 'helpUsers':
        await bot.send_message(callback_query.from_user.id, text ='Чтобы узнать больше, обратитесь к данному админу : @malch2ik')
    elif callback_query.data == 'paymentSystem':
        await bot.send_message(callback_query.from_user.id, text = 'На данный момент поддерживается только QIWI.')
    elif callback_query.data == 'info':
        await bot.send_message(callback_query.from_user.id, text = """Продажа ключей:
...
4. При получении платежа бот автоматически вышлет вам обмен с ключами.""")
        ...
        buy, sold = keyses()
        await bot.send_message(callback_query.from_user.id, text = f'''Купить ключи за {buy} Р/шт
...
После того как вы отправили обмен, подтвердили трейд, отправьте после этого сообщения слово *Готово*, в ином случае вам придется делать процедуру заново''', parse_mode = "Markdown")
        await send.sellKeys.set()
    elif callback_query.data == 'buyTf':
        await bot.send_message(callback_query.from_user.id, 'Выберите метод оплаты', reply_markup= keyboard.paymentChoise)
                            ...
                            for j in range(len(i['items_to_receive'])):
                                if i['items_to_receive'][0]['classid'] == "101785959":
                                    ...
                                    if total == len(i['items_to_receive']):
                                        ...
                                        steam_client.accept_trade_offer(steam_trade_offer)
                                        time.sleep(3)
                                        await bot.send_message(message.from_user.id, f'Ваши ключи в количестве {total} были приняты, ожидайте пополнения баланса в профиле')
                                        add_money_user(total_price, message.from_user.id)
                                        buyKeys(int(total))
                                        userNameTg = message.from_user.first_name
                                        userIdTg = message.from_user.id
                                        await bot.send_message('878562927', f'{userNameTg}({userIdTg}) купил ключи в количестве {kolvo_keys_TF2} штук за {total_price} рублей.')
                                        await bot.send_message(message.from_user.id, "Денежные средства были успешно добавлены в профиль. Для вывода зайдите в свой профиль, и запросите вывод сресдтв")
                                        await bot.send_message(message.from_user.id, '''Спасибо за покупку/продажу, будем рады вам снов а!
Оставьте отзыв в нашей теме : https://lolz.guru/threads/3443335/
Это помогает наш ему развитию, спасибо вам!''')
                                        break
                                else:
                                    await bot.send_message(message.from_user.id, 'В трейде обнаружены лишние вещи, трейд был отменен, сделайте процедуру заново, будьте внимательны')
                                    steam_client.decline_trade_offer(steam_trade_offer)
                                    break
                    else:
                        await bot.send_message(message.from_user.id, f'Трейда с комментарием {gener()} не найдено')
                        break
            else:
                await bot.send_message(message.from_user.id, "Обратить к тех поддержке")
        else:
            await bot.send_message(message.from_user.id, 'Вы не ввели слово ГОТОВО, начните заново!')
    except ApiException:
...
async def choisePay(message: types.Message, state: FSMContext):
    if message.text == 'Отмена':
        ...
        await message.answer('Выбрана оплата через LOLZ', reply_markup= keyboard.userButtonMenu)
        try:
            await bot.send_message(message.from_user.id, text = f'Ключей в наличии : {inv_steam()}')
            if int(inv_steam()) >= int(1):
                await bot.send_message(message.from_user.id, text = "Введите свой трейд url и количество ключей для покупки(Пример : *URL* *5*)", parse_mode='Markdown')
                await send.nextBuyTf.set()
            else:
                await bot.send_message(message.from_user.id, text = 'К сожалению мы не обладаем ключами для продажи')
        except AttributeError:
                await bot.send_message(message.from_user.id, text = 'Недостаточно ключей')
                logging.error(f'{message.from_user.id} | {message.text}')
    else:
        await message.answer('Выбрана оплата через QIWI', reply_markup= keyboard.userButtonMenu)
        try:
            await bot.send_message(message.from_user.id, text =  f'Ключей в наличии : {inv_steam()}')
            if int(inv_steam()) >= int(1):
                await bot.send_message(message.from_user.id, text = "Введите свой трейд url и количество ключей для покупки(Пример : *URL* *5*)", parse_mode='Markdown')
...
async def nextBuyTf(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if int(total) <= int(inv_steam()):
            ...
            await message.answer(f'❗️ Важно! Не меняйте сумму оплаты и комментарий, иначе зачисление не будет произведено!\n\nСсылка для оплаты находится ниже 👇', reply_markup=keyboard.check_code(comment, int(price), url, total, urlLink))
        else:
            await message.answer(f'❗️Ключей в количестве {total} штук у нас нет. Попробуйте меньшее число')
    except ValueError:
...
async def buyTfQiwi(call: CallbackQuery, message: types.Message):
    try:
        ...
        if checkQiwi(comment, total) == True:
            buy, sold = keyses()
            if int(counter) >= 1:
                if session == True:
                    ...
                    items_for_trade = []
                    for key, v in my_items.items():
                        if v['classid'] == '101785959':
                            items_for_trade.append(Asset(v['id'], game))
                    steam_client.make_offer_with_url(items_for_trade[:int(counter)],[], url, gener())
                    time.sleep(3)
    ...
    Это помогает нашему разитию, спасибо вам!''')
                    ...
                    userNameTg = message.from_user.first_name
                    userIdTg = message.from_user.id
                    await bot.send_message('878562927', f'{userNameTg}({userIdTg}) купил ключи в количестве {int(counter)} штук за {counter} рублей.')
                else:
                    await bot.send_message(message.from_user.id, "Напишите @malchik код 1")
            elif int(counter) == 0:
                await bot.send_message(message.from_user.id, '0 ключей купить нельзя. Напишите реальное кол-во ключей для покупки')
        else:
            await bot.send_message(message.from_user.id, '❗️ К сожалению ваш платеж не обнаружен')
    except ValueError:
        await bot.send_message(message.from_user.id, "Вы ввели некорректное число!")
        logging.error(f'{message.from_user.id} | {message.text}')
    except QIWIAPIError:
        await bot.send_message(message.from_user.id, 'Входящего платежа не найдено. Проверьте кому вы отправили деньги')
        logging.error(f'{message.from_user.id} | {message.text}')
    except AttributeError:
        bot.send_message(message.from_user.id, 'Нет ключей')
        logging.error(f'{message.from_user.id} | {message.text}')
    except IndexError:
        await bot.send_message(message.from_user.id, 'Прочитай инструкцию, в вспылающем окне, перед отравкой *URL* и *кол-во ключей* и пробуйте продать ключи ещё раз', parse_mode= 'Markdown')
        logging.error(f'{message.from_user.id} | {message.text}')
...
async def buyTfLolz(call: CallbackQuery, message: types.Message):
    try:
        ...
        counter = call_data[3]
        url = 'https://steamcommunity.com/tradeoffer/new/' + call_data[4]
        if check_pay(comment, total) == True:
            await bot.delete_message(chat_id=message.from_user.id, message_id=message.message.message_id)
            buy, sold = keyses()
            if int(counter) >= 1:
    ...
    Это помогает нашему разитию, спасибо вам!''')
                    userNameTg = message.from_user.first_name
                    userIdTg = message.from_user.id
                    await bot.send_message('878562927', f'{userNameTg}({userIdTg}) купил ключи в количестве {int(counter)} штук за {price} рублей.')
                else:
                    await bot.send_message(message.from_user.id, "Напишите @malchik код 1")
            elif int(counter) == 0:
                await bot.send_message(message.from_user.id, '0 ключей купить нельзя. Напишите реальное кол-во ключей для покупки')
...
async def nextBuyTf(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if int(total) <= int(inv_steam()):
            ...
            await message.answer(f'❗️ Важно! Не меняйте сумму оплаты и комментарий, иначе зачисление не будет произведено!\n\nСсылка для оплаты находится ниже 👇', reply_markup=keyboard.check_codeQiwi(comment, price, url, total, urlLink))    
        else:
            await message.answer(f'❗️Ключей в количестве {total} штук у нас нет. Попробуйте меньшее число')
    except ValueError:
        await message.answer('❗️Не правильное кол-во ключей')
...
async def newsLetter(message : types.Message, state: FSMContext):
    ...
    for i in range(len(info)):
        try:
            ...
            continue
    await bot.send_message(message.from_user.id, 'Рассылка закончена')
    await state.finish()
...
async def sellTf(message : types.Message, state: FSMContext):
    try:
        ...
        if float(money) < float(message.text.split()[1]):
            await bot.send_message(message.from_user.id, 'У вас нет столько средств на балансе для вывода')
        else:
            await bot.send_message(message.from_user.id, f'{message.from_user.username}, вывод успешно запрошен ожидайте денег от 30 минут до 24 часов')
            await bot.send_message(878562927, f'Пользователь {message.from_user.id} запросил вывод средст на номер {message.text.split()[0]} сумму {message.text.split()[1]}')
            block(message.from_user.id, float(message.text.split()[1]))
        await state.finish()
    except ValueError:
        await bot.send_message(message.from_user.id, 'Напишите кол-во средств через .\n(Например 499.1)')
    except:
        logging.error(f'{message.from_user.id} | {message.text}')
