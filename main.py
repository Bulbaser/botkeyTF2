...
async def admin(message: types.Message):
    if message.from_user.id == int('878562927') or message.from_user.id == int('1059195416'):
        await message.answer(f'Привет, {message.from_user.username}. Ты авторизовался в админке',
...
async def some_callback_handler(callback_query: types.CallbackQuery):
    if callback_query.data == 'withdraw':
        ...
        await bot.send_message(callback_query.from_user.id,
                               text='Чтобы узнать больше, обратитесь к '
                                    'данному админу : @malch2ik')
    ...
    try:
        ...
        if int(total) <= int(inv_steam()):
            ...
            await message.answer(
                ...
                reply_markup=keyboard.check_code(comment, int(price), url, total, urlLink))
        else:
...
async def buyTfLolz(call: CallbackQuery, message: types.Message):
        ...
        if check_pay(comment, total):
            ...
            if int(counter) >= 1:
                ...
                if session:
                    ...
                    for key, v in my_items.items():
                        if v['classid'] == '101785959':
                            items_for_trade.append(Asset(v['id'], GameOptions.TF2))
                    steam_client.make_offer_with_url(items_for_trade[:int(counter)], [], url, gener())
                    time.sleep(3)
                    soldKeys(int(counter))
                    await bot.send_message(message.from_user.id, "Обмен отправлен")
    ...
    Это помогает нашему разитию, спасибо вам!''')
                    ...
                    userIdTg = message.from_user.id
                    await bot.send_message('878562927',
                                           f'{userNameTg}({userIdTg}) купил ключи в количестве '
        ...
        await bot.send_message(message.from_user.id,
                               'Прочитай инструкцию, в вспылающем окне, перед отравкой *URL* и *кол-во ключей* и '
                               'пробуйте продать ключи ещё раз', parse_mode='Markdown')
        logging.error(f'{message.from_user.id} | {message.text}')
    ...
    try:
        ...
        if int(total) <= int(inv_steam()):
            ...
            await message.answer(
                f'❗️ Важно! Не меняйте сумму оплаты и комментарий, иначе зачисление не будет произведено!\n\nСсылка '
                f'для оплаты находится ниже 👇',
                reply_markup=keyboard.check_codeQiwi(comment, price, url, total, urlLink))
        else:
            await message.answer(f'❗️Ключей в количестве {total} штук у нас нет. Попробуйте меньшее число')
    except ValueError:
        await message.answer('❗️Не правильное кол-во ключей')
...
async def newsLetter(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, f''
                                                 f'Активные пользователи бота: {success_send_email}'
                                                 f'Мертвые пользователи: {fail_send_email}')
    await state.finish()
